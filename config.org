...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package circe
    ...
    (progn
      ...
      (use-package pass :demand t :ensure t)
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package company
    ...
    :init
    (progn
      (use-package company-emoji :demand t :ensure t)
...
** ~company-lsp~                                                     :melpa:
#+BEGIN_SRC emacs-lisp
  (use-package company-lsp :after company :demand t :ensure t)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package dockerfile-mode :demand t :ensure t)
#+END_SRC

** ~dtrt-indent~                                                     :melpa:
#+BEGIN_SRC emacs-lisp
  (use-package dtrt-indent :init (dtrt-indent-mode) :demand t :ensure t)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun eqyiel/company-js2-mode-hook ()
    ;; Do not replace tide's company configuration if it is active
    (unless (eqyiel/ts-should-enable)
      (set (make-local-variable 'company-backends)
           '((company-files
              company-lsp
              company-keywords)))))
  ...
  (defun eqyiel/flycheck-js2-mode-hook ()
    (require 'flycheck)
  ...
  (use-package js2-mode
    ...
    (progn
      ...
      (use-package flow-js2-mode
        ...
        :config (use-package flow-minor-mode :demand t :ensure t)
        :demand t)
    ...
    :init (progn
            ...
            (flycheck-add-mode 'javascript-flow-coverage 'js2-mode))
    :hook ((js2-mode-hook . eqyiel/flycheck-js2-mode-hook)
           (js2-mode-hook . eqyiel/company-js2-mode-hook)
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package
    flow-minor-mode
    :hook (js2-mode-hook . eqyiel/lsp-javascript-flow-enable-maybe)
    :demand t)
...
#+BEGIN_SRC emacs-lisp
  (use-package magit
    ...
    ((git-commit-mode-hook
      ;; > git-commit-mode is a minor-mode but you have limited the snippets to be
      ;; > used with a major-mode git-commit-mode. Such a major-mode no longer
      ;; > exists, so you have to use a different mechanism to restrict your
      ;; > snippets to "when I edit a commit message".
      ;;
      ;; > The reason git-commit-mode is a minor-mode is to allow you to choose
      ;; > the major-mode most suitable for commit messages according to the
      ;; > conventions used by the projects you contribute to. By default this is
      ;; > text-mode, but if some project uses Markdown or Org-Mode for commit
      ;; > messages, then you can configure that on a per project basis.
      ;; >
      ;; > Assuming you have some snippets that are appropriate for
      ;; > text-mode+git-commit-mode, but not just text-mode or say
      ;; > markdown-mode+git-commit-mode, then use something like:
      ;;
...
** ~markdown-mode~                                                   :melpa:
#+BEGIN_SRC emacs-lisp
  (defun eqyiel/remark-lint-error-parser (output checker buffer)
    (let ((parsed-output (caar (flycheck-parse-json output))))
      (seq-map
       (lambda (message)
         (let-alist message
       ...
       (alist-get 'messages parsed-output))))
  ...
  (use-package markdown-mode
    ...
    (progn
      ...
      (use-package flycheck
        ...
        (progn
          (require 'warnings)
          (flycheck-define-checker remark-lint
            "Flycheck plugin for markdown-mode"
            :command ("remark-lint" "--report" "json" source-inplace)
            :error-parser eqyiel/remark-lint-error-parser
            :modes markdown-mode))
        :ensure t))
    :init (add-to-list 'flycheck-checkers 'remark-lint)
...
** ~mode-line-bell~                                                  :melpa:

Flash the mode line instead of ringing the terminal bell.
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package mu4e
    :config
    (progn
      (use-package org-mime :demand t :ensure t)
...
#+BEGIN_SRC emacs-lisp
(use-package org-cliplink :bind (("H-i" . org-cliplink)) :ensure t)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package org
    ...
    (setq
     ...
     `(("t" "Todo" entry (file ,(eqyiel/create-org-file-path "inbox.org"))
        "* TODO %?
  SCHEDULED: %T
  :PROPERTIES:
  :CREATED: %U
...
#+BEGIN_SRC emacs-lisp
  (use-package ox-gfm :demand t :ensure t)
#+END_SRC
...
** ~rainbow-mode~                                                     :elpa:
#+BEGIN_SRC emacs-lisp
  (use-package rainbow-mode :commands rainbow-turn-on :ensure t)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package scss-mode
    :hook (scss-mode-hook . eqyiel/scss-mode-hook) :ensure t)
#+END_SRC
     ...
     `(counsel-rg-base-command
       ,(concat
         (s-replace-regexp
          "[\[:space:]]?%s[\[:space:]]?.?"
          " --follow %s ."
          (string-join (eval (car (get 'counsel-rg-base-command 'standard-value))) " "))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package tide
    ...
    :hook (((js2-mode-hook web-mode-hook typescript-mode-hook) . eqyiel/ts-tide-enable-maybe)
