...
#+BEGIN_SRC emacs-lisp
  ...
  (defun eqyiel/node-package-path-exists-p (js-project-root resolved-dependency)
    "Construct the path in JS-PROJECT-ROOT to RESOLVED-DEPENDENCY."
                     ...
                     (delq nil
                           (mapcar
                            ...
                            (if (listp dependency)
                                dependency (list (intern dependency))))))
                  (cdr (assoc 'bin package-file-as-string)))))
          (if (symbolp local-project-executable)
