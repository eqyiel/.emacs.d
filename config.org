...
#+BEGIN_SRC emacs-lisp
  (defun eqyiel/locate-js-project-root (path)
  ...
  (defun eqyiel/find-prettier-in-package-file (package-file)
  ...
  (defun eqyiel/should-format-with-prettier-p ()
    "Return t if prettier found in both package.json and node_modules."
    (let* ((js-project-root (eqyiel/locate-js-project-root default-directory))
           (node-modules-dir (concat js-project-root "/node_modules"))
           (prettier-package (eqyiel/find-prettier-in-package-file
                              (concat js-project-root "/package.json")))
           (prettier-package-path (concat node-modules-dir "/" prettier-package)))
      (and prettier-package
           (file-readable-p prettier-package-path)
           (file-exists-p prettier-package-path))))

  (defun eqyiel/prettier-js-mode-hook ()
    "Maybe format buffer with prettier on save."
    (when (eqyiel/should-format-with-prettier-p) (prettier-js-mode)))
  ...
  (use-package prettier-js
    :init (progn
            ...
            (add-hook 'web-mode-hook
                      (lambda ()
                        (when (and (or (string-equal web-mode-content-type "javascript")
                                       (string-equal web-mode-content-type "jsx"))
                                   (eqyiel/should-format-with-prettier-p))
                          (prettier-js-mode)))))
